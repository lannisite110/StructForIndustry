#!/usr/bin/env bash
# V4L2 trigger E2E: TRIG → v4l2-capture → HAL → mock defect-detect.
set -euo pipefail
...
if [[ ! -e "$DEVICE" ]]; then
  echo "skip v4l2 trigger E2E: $DEVICE not present"
  exit 0
fi

RUN_DIR="${XDG_RUNTIME_DIR:-/tmp}/sfi-v4l2-trig-e2e"
mkdir -p "$RUN_DIR"
BUS_SOCK="$RUN_DIR/bus.sock"
VISION_SOCK="$RUN_DIR/vision.sock"
PLC_SOCK="$RUN_DIR/plc.sock"
HTTP_ADDR="127.0.0.1:18184"
SHM_NAME="sfi.v4l2.trig.e2e"
...
echo "v4l2 trigger E2E OK ($DEVICE)"
