...
use opcua::{parse_config, BoolReader};
...
async fn main() -> std::io::Result<()> {
    ...
    let bus_socket: PathBuf = std::env::var("SFI_BUS_SOCKET")
        ...
        .into();
    let shm_name = std::env::var("SFI_LINE_SHM").unwrap_or_else(|_| "sfi.opcua.line.0".into());
    ...
    let mut reader = BoolReader::connect(&config).await?;

    let mut prev = false;
    let poll = Duration::from_millis(config.poll_ms.max(10));

    loop {
        let val = reader.read_bool().await?;
        if val && !prev {
            ...
            tracing::info!(frame_id = id, "OPC-UA rising edge → HAL");
        }
        prev = val;
        sleep(poll).await;
    }
